...
def testAccuracy_MC(sr_base, sr_case):
    ...
    _pathOutput = 'H:/Studies/Quant #23/paper/'
    #1) Determine mu,sigma
    mu_base, sigma_base = sr_base/252, 1/252**.5
    mu_case, sigma_case = sr_case/252, 1/252**.5
    hist, probOverfit = [], 0
    #2) Generate trials
    for m in range(numMC):
        ...
        mu_is = [np.average(pnl[:int(length/2), i]) for i in range(pnl.shape[1])]
        sigma_is = [np.std(pnl[:int(length/2), i]) for i in range(pnl.shape[1])]
        mu_oos = [np.average(pnl[int(length/2):, i]) for i in range(pnl.shape[1])]
        sigma_oos = [np.std(pnl[int(length/2):, i]) for i in range(pnl.shape[1])]
